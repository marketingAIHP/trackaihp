...
const wrap = (input: string, width: number, size = 7) => {
  ...
  String(input || '').replace(/\r/g, '').split('\n').forEach((paragraph) => {
    ...
    paragraph.split(/\s+/).filter(Boolean).forEach((word) => {
      ...
      else { if (current) result.push(current); if (word.length <= maxChars) current = word; else { for (let i = 0; i < word.length; i += maxChars) result.push(word.slice(i, i + maxChars)); current = ''; } }
    });
...
export function buildTimelineReportPdf(rows: TimelineReportRow[], meta: TimelineReportMeta) {
  ...
  const weights: Record<string, number> = { 'Start Time': 0.9, 'End Time': 0.9, Duration: 0.8, Status: 1.05, 'Location Name': 1.5, 'Site Name': 1.4, 'Checkout Type': 1.15 };
  ...
  type Segment = { kind: 'day'; label: string } | { kind: 'row'; layout: number; start: number; count: number };
  const daySectionHeight = 18;
  const pages: Segment[][] = [[]]; let pageIndex = 0, cursor = firstTableTop;
  let currentDay = '';
  layouts.forEach((layout, layoutIndex) => {
    const nextDay = formatDate(orderedRows[layoutIndex].event_time);
    if (nextDay !== currentDay) {
      ...
      if (cursor - daySectionHeight - headerHeight - lineHeight - padding * 2 < bottom) {
        pages.push([]); pageIndex++; cursor = continuationTableTop;
      }
  ...
  const streams = pages.map((segments, index) => {
    const isFirstPage = index === 0; let out = '';
    if (isFirstPage) {
    ...
    segments.forEach((segment) => {
      ...
      if (!hasTableHeader) drawColumnHeader();
      ...
      out += line(margin + usableWidth, y, margin + usableWidth, y - height) + line(margin, y - height, margin + usableWidth, y - height); y -= height;
    });
