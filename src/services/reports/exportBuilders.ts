...
function getAttendanceReportColumns(): AttendanceReportColumn[] {
  return [
    ...
    { key: 'latitude', header: 'Latitude', widthWeight: 0.95, getValue: (row) => formatCoordinate(row.latitude) },
    { key: 'longitude', header: 'Longitude', widthWeight: 0.95, getValue: (row) => formatCoordinate(row.longitude) },
    {
      key: 'checkout_type',
...
function buildReportHeaderLines(meta: AttendanceReportExportMeta, pageWidth: number, isFirstPage: boolean) {
  ...
  lines.push({ text: `Total Records: ${meta.totalRecords}`, font: 'F1', size: 10, gapAfter: isFirstPage ? 10 : 8 });
