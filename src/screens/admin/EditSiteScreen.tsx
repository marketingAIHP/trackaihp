...
export const EditSiteScreen: React.FC = () => {
  ...
  const {data: site, isLoading} = useQuery({
    ...
    queryFn: async () => {
      const response = await adminApi.getSites(currentUser?.id || 0);
      if (response.success && response.data) {
        const foundSite = response.data.find((s) => s.id === siteId);
        if (foundSite) {
          originalImageUrlRef.current = foundSite.site_image || null;
          setSelectedImageUri(foundSite.site_image || null);
          return foundSite;
        }
      }
      throw new Error('Site not found');
    },
    enabled: !!siteId && !!currentUser?.id,
  });
  ...
  const updateSiteMutation = useMutation({
    mutationFn: async (data: SiteFormData) => {
      if (!currentUser?.id || !siteId) {
        throw new Error('You must be logged in to update sites');
      }
      ...
      if (selectedImageUri) {
        if (selectedImageUri.startsWith('http://') || selectedImageUri.startsWith('https://')) {
          ...
          try {
            ...
            if (originalImageUrlRef.current && originalImageUrlRef.current !== selectedImageUri) {
              await deleteImage(originalImageUrlRef.current, STORAGE_BUCKETS.SITE_IMAGES);
            }
        ...
        // Image removed - delete original if it existed
      ...
      const response = await adminApi.updateSite(currentUser.id, siteId, {
        name: data.name,
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({queryKey: ['admin', 'site', siteId]});
    ...
    return (
      <SafeAreaView style={[styles.container, {backgroundColor: theme.colors.background}]}>
        <View style={styles.errorContainer}>
          <Text variant="bodyLarge">Site not found</Text>
        ...
        <Card style={styles.formCard}>
          <Card.Content>
            ...
            <Button
              ...
              loading={isSubmitting || updateSiteMutation.isPending}
              disabled={isSubmitting || updateSiteMutation.isPending}
